...
참고합니다. 자문 원본 요청, AI 서술, URL, 근거 ID, 도메인, 제공처 이름, 인증 정보는 이 컨텍스트와
`report_inputs` 스냅샷에 복사하지 않으며, 자문 저장소를 조회할 수 없어도 기존 리포트 생성은
계속됩니다.

뉴스 헤드라인은 내부 입력으로 계속 사용하지만 리포트 본문에는 URL, 도메인, 뉴스 사이트명 또는
`GDELT`를 표시하지 않습니다. 모델은 뉴스에 근거한 문장 끝에 `[[N1]]` 형태의 근거 마커만 남기고,
백엔드가 저장 직전에 마커와 남아 있는 출처 표기를 제거합니다(`fact_enforcer.redact_source_references`).
검열은 저장 전에 수행되므로 API 응답·알림·내보내기 등 모든 소비자가 동일한 본문을 받습니다.
근거 마커 덕분에 어떤 기사가 실제로 인용됐는지는 `report_inputs.news_context.evidence_usage`에
그대로 남습니다.
...
- 8개 자문 유형은 각각 전용 한국어 표·모바일 카드·단위·상태 배지·SEC 공시 링크·AI 설명과 근거 ID를 표시합니다. `partial`, `limited`, `data-limited`, `insufficient_data` 상태와 N-PORT 공시 지연 안내를 결과보다 먼저 표시합니다. 데이터 기준일·제공처·데이터 상태·누락 필드·제한 사항은 항상 노출하고, 근거 레코드 목록만 접이식 영역에 둡니다.
