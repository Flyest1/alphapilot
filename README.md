...
삭제되거나 수정되지 않습니다. 022는 한 계좌의 Toss 보유 항목 upsert와 최신 응답에서 사라진
항목의 0수량 처리를 단일 데이터베이스 트랜잭션으로 수행하는 함수를 추가합니다.
...
- **데이터 무결성**: 시장 값이 `KR` 또는 `US`가 아닌 보유 항목은 저장하지 않으며, 한 계좌의
  보유 항목 갱신과 0수량 처리는 모두 성공하거나 모두 취소됩니다.
