...
원장 오류 해결·중복 거래 검토·원본 백업 도구는
[원장 복구 운영 안내](docs/ledger_recovery_2026_09_23.md)를 참고하세요.
검토는 원본을 변경하지 않으며 실제 계좌 수익률 검증을 대신하지 않습니다.

국내·글로벌 리포트는 최근 30일 내 완료된 AI 자문 중 유형별 최신 결과를 제한된 내부 컨텍스트로만
참고합니다. 자문 원본 요청, AI 서술, URL, 근거 ID, 도메인, 인증 정보는 이 컨텍스트와
`report_inputs` 스냅샷에 복사하지 않으며, 자문 저장소를 조회할 수 없어도 기존 리포트 생성은
계속됩니다. 뉴스 헤드라인은 내부 입력으로 계속 사용하지만 리포트 본문에는 근거 ID, URL, 도메인,
뉴스 사이트명 또는 `GDELT`를 표시하지 않습니다.
...
이익실현 판단은 저장된 보유 자산의 원가·수량과 최신 시세를 사용해 `SELL`, `REDUCE`, `HOLD`,
`BUY`, `WATCH`를 독립적으로 비교합니다. 최신 리포트의 매수 의견은 충돌 설명에만 사용하며 새
판단 점수에는 반영하지 않습니다. 결과의 가격 기준은 관찰·재검토용이고 수익을 보장하거나
매도·매수 주문을 실행하지 않습니다.
...
Toss Invest로 동기화된 자산은 `Toss 연동` 배지와 동기화 시간이 표시됩니다. 기존에 같은 종목을 수동으로 입력해 둔 경우 동기화 결과에 중복 후보가 표시되며, 확인 후 수동 자산을 직접 삭제해 중복 계산을 피할 수 있습니다. 정상 동기화에서 미보유로 확인되거나 수량이 0인 해당 계좌의 Toss 연동 자산은 자산 목록에서 삭제됩니다. 과거 리포트·전략·성과 기록·추천 사이클·포트폴리오 스냅샷은 보존됩니다. 수동 자산과 다른 계좌의 자산은 변경하지 않습니다.
...
시세를 조회할 수 없거나 시세가 오래된 투자 자산은 평균 매입가로 대체하지 않습니다.
대시보드에 평가 가능한 자산 수와 확인된 부분 평가액을 표시하고, 전체 평가액·평가손익·수익률은
`계산 불가`로 표시합니다. CASH의 입력 금액 계산 방식은 유지됩니다.

API 소비자는 `GET /api/portfolio/summary`의 `total_market_value`, `total_profit_loss`,
`total_return_rate`, `total_net_profit_loss`, `total_net_return_rate`에 `null`이 올 수 있음을
처리해야 합니다. `domestic_value`와 `global_value`는 해당 시장의 평가가 불완전하면 `null`이며,
`daily_profit_loss`와 `daily_return_rate`는 이전 종가가 부족해도 `null`입니다. `null`은 0이 아닙니다.
`valuation_status`는 `complete`, `partial`, `unavailable`이며, `valued_market_value`,
`valued_asset_count`, `total_asset_count`가 확인된 평가 범위를 나타냅니다. 자산별 행은
`valuation_status`(`current`, `stale`, `missing`), `provider`, `last_trading_date`,
`last_observed_price_native`를 제공합니다. 오래된 관측 가격은 현재 평가액에 포함되지 않습니다.
리포트 JSON의 `portfolio_summary.total_market_value`와 `total_return_rate`도 필드는 유지하되
계산할 수 없으면 `null`이며, data-limited 설명과 위험 안내를 제공합니다.

전체 평가 또는 일간 변동을 계산할 수 없으면 새 스냅샷을 저장하지 않습니다. 가격 이력이 없는
자산을 매입원가로 그리지 않으며, 불완전한 평가에서 비중·리밸런스 제안은 표시하지 않습니다.
과거 스냅샷은 자동 수정하지 않았으므로 당시 시세 대체나 자산 구성 변화의 영향이 남을 수 있습니다.
현재 수량·현재 환율로 재구성한 가격 이력과 스냅샷 평가액 변화는 입출금·거래를 조정한 실제
계좌 수익률(TWR/IRR)이 아닙니다. 조회한 환율을 사용할 수 없으면 설정 환율을 사용하는 기존
방식을 유지하며, 별도의 환율 품질 상태는 아직 제공하지 않습니다.
...
`비교` 탭의 `추천 이후 평균 가격 변화`는 최근 50개 추천 기준가 대비 단순 가격 변화 평균입니다. 매수·매도 방향, 비용, 투자 비중을 반영하지 않고 날짜별 표본도 달라집니다. `보유자산 평가액 변화`는 저장된 스냅샷의 총 평가액 변화이며 입출금·매매·환율의 영향을 포함합니다. 둘 다 실제 계좌 운용 수익률로 해석하지 않습니다. 미국 증시 대표선은 S&P 500을 사용합니다.
...
backend/app/db/migrations/022_add_atomic_toss_holdings_reconciliation.sql
backend/app/db/migrations/024_delete_closed_toss_holdings.sql
```

014는 `assets` 테이블에 수동/외부 연동 출처, 외부 계좌/종목 키, 동기화 시간, 원본 payload
컬럼과 중복 방지 인덱스를 추가합니다. 기존 수동 자산은 `source='manual'` 기본값을 가지며,
삭제되거나 수정되지 않습니다. 022는 선택한 계좌의 Toss 보유 항목 upsert, 최신 응답에서 사라진
항목과 이전에 선택했던 다른 계좌 잔고의 0수량 처리를 단일 데이터베이스 트랜잭션으로 수행하는
함수를 추가합니다. 024는 이 함수를 교체하여 정상 동기화된 계좌의 미보유·0수량 자산을
삭제하며, 다른 계좌는 변경하지 않습니다. 기존 0수량 자산도 해당 계좌의 다음 정상 동기화에서
정리됩니다. 마이그레이션 실행 자체는 자산을 삭제하지 않습니다. 과거 전략의 `asset_id`만
`NULL`로 분리되고 리포트·전략·성과 기록·추천 사이클·스냅샷은 유지됩니다.
운영 반영에는 Supabase SQL Editor에서 024를 수동 적용해야 합니다. 코드 배포만으로는
기존 DB 함수의 0수량 보존 동작이 바뀌지 않습니다.
...
15개 종목을 추가해 기본 미국 주식 유니버스를 30개로 확장합니다. 020은 기존 자문 테이블의
...
프론트엔드 `리포트` 화면에서 국내/글로벌 리포트를 직접 생성할 수 있습니다. 수동 생성 API는 `API_ACCESS_TOKEN`으로 보호됩니다. 수동 리포트는 Toss 보유주식 동기화를 실행하지 않고 현재 저장된 보유 정보를 바로 사용합니다.
...
- **리포트 연계**: GitHub Actions가 호출하는 정기 리포트는 먼저 Toss 보유주식을 동기화하고,
  ...
  건너뛰고 저장된 수동 자산으로 정기 리포트를 생성합니다. 단, 양수 수량의 Toss 연동 자산이
  남아 있는 상태에서 인증정보를 제거하면 오래된 잔고 사용을 막기 위해 정기 작업을 실패
  처리합니다. 화면에서 요청한 수동 리포트는 동기화를 건너뜁니다.
- **동시 실행**: 직접 요청한 Toss 동기화와 모든 리포트 생성은 같은 실행 잠금을 사용합니다.
  리포트 생성이나 다른 동기화가 진행 중이면 직접 동기화 요청은 `409 Conflict`로 종료되며,
  기존 자산은 변경하지 않습니다.
- **미보유 처리**: 정상 동기화에서 사라졌거나 수량이 0인 해당 계좌의 Toss 연동 자산은
  삭제합니다. 수동 자산과 다른 계좌는 유지하며, 과거 리포트·전략·성과 기록·추천 사이클·
  스냅샷도 보존합니다. `synced_count`는 양수 보유 항목 수이며, 기존 응답 필드 `stale_count`는
  삭제된 자산 수를 나타냅니다. 정상 응답의 빈 보유 목록은 해당 계좌의 전체 미보유로 처리하며,
  응답 실패나 유효하지 않은 데이터가 있으면 삭제하지 않습니다.
- **데이터 무결성**: 시장 값이 `KR` 또는 `US`가 아닌 보유 항목과 시장·통화가 `KR`-`KRW` 또는
  `US`-`USD`로 일치하지 않는 항목은 저장하지 않습니다. 선택 계좌의 보유 항목 갱신, 최신
  응답에서 미보유·0수량으로 확인된 해당 계좌 자산의 삭제는 모두 성공하거나 모두 취소됩니다.
