-- 025: include KRW/USD available cash in atomic Toss reconciliation. No data changed on apply.
-- Replaces 024 without deleting rows at migration time. The next successful sync
...
begin
  ...
  if exists (
    ...
    where input_asset.market is null
      or input_asset.market not in ('KR', 'US', 'CASH')
      ...
      or input_asset.currency is distinct from
        case when input_asset.market = 'KR' then 'KRW'
             when input_asset.market = 'US' then 'USD' else input_asset.ticker end
      or (input_asset.market = 'CASH' and (
        input_asset.ticker not in ('KRW', 'USD') or input_asset.avg_price <> 1
      ))
