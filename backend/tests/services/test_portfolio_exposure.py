...
class CurrentQuotes:
    def fetch_price_history(self, market, ticker):
        price = 70000 if ticker == "005930" else 100 if ticker == "JPM" else 100000
        return SimpleNamespace(current_price=price, is_stale=False, dataframe=pd.DataFrame())
...
def test_summary_includes_currency_market_sector_exposure():
    ...
    # 명시적으로 제공된 현재 시세 기준: 삼성 700,000 + JPM 100,000 + 현금 100,000
