...
- [x] 설계 문서의 테이블·키·정밀도·접근 정책을 DDL로 고정하고 AGENTS.md에 반영.
- [x] 동일 이벤트 동시 입력, 중간 실패 rollback, 다른 계좌 충돌 방지 테스트 작성.
- [x] 원본 저장과 검증 이벤트 게시를 트랜잭션으로 분리하고 부분 수집 상태 보존.
- [x] 기존 assets와 FK·코드 경로를 분리하고 기존 삭제 회귀 및 원장 불변성 검증 후 scoped commit.

작업 3 구현 완료: migration 026, 로컬 PostgreSQL 실제 통합 검증과 전체 719개 테스트 통과.
원문은 로컬 Git 제외 경로에 보존하고 DB에는 출처 메타데이터만 저장한다.
공통 CSV 매핑 CLI도 추가했으며 실제 명세서는 아직 없다고 사용자가 확인했다.
운영 migration은 미적용. 상세 결정과 제한은 `docs/account_ledger_storage_2026_09_11.md` 참조.
...
계약은 당시 남은 범위였다. 이후 작업 3의 로컬 구현은 위 결과로 갱신했다.
상세 범위는 `docs/local_account_ledger_2026_09_11.md`를 참조한다.
