...
- [ ] 숫자 문자열·통화·일시·출처 해시 검증의 실패 테스트 작성.
- [ ] null 비용과 0 비용, 날짜만 있는 증거와 정확한 시각, 정밀도 초과를 구분.
- [ ] 같은 문서 재입력과 동일 내용의 서로 다른 실제 거래를 구분하는 fixture 추가.
- [ ] 모델·검증기를 구현하고 전체 검사 후 `feat: validate account ledger evidence` 커밋.
...
- [ ] 누적 체결 2→5 재조회가 총 7주가 되지 않는 테스트 작성.
- [ ] 수정·취소 분개, 주문 대체 연결 미확인, 수수료 null의 격리 테스트 작성.
- [ ] 매매일/결제일 구분과 환전 양 leg, 배당 세전/세금/실입금 대사 구현.
- [ ] 기초 원가 없을 때 실현손익 null, 매수 가능액을 정산 현금으로 거부하는 테스트 추가.
- [ ] 전체 검사 후 `feat: replay and reconcile account ledger evidence` 커밋.
...
이번 설계 작업에서 위 구현 단계가 완료됐다고 표시하지 않는다. 다음 실행 단위는 작업 1~2이며,
아직 제공되지 않은 실제 명세서 대신 명시적으로 가상인 fixture로 시작할 수 있다.
